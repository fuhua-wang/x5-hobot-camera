	...
	if (ret < 0) {
        ...
        switch(sensor_info->sensor_mode) {
                case NORMAL_M:	  // 1: normal
                        vin_info("sc035hgs in normal mode\n");
			setting_size = sizeof(sc035hgs_linear_init_setting) / sizeof(uint32_t) / 2;
			...
			ret = vin_write_array(sensor_info->bus_num, sensor_info->sensor_addr, 2,
						setting_size, sc035hgs_linear_init_setting);
...
int sc035hgs_linear_data_init(sensor_info_t *sensor_info)
{
	...
	if (turning_data.normal.again_lut != NULL) {
		...
		for (open_cnt =0; open_cnt <
			sizeof(sc035hgs_again_lut0)/sizeof(uint32_t); open_cnt++) {
	...
	if (turning_data.normal.dgain_lut != NULL) {
		...
		for (open_cnt =0; open_cnt <
			sizeof(sc035hgs_dgain_lut0)/sizeof(uint32_t); open_cnt++) {
